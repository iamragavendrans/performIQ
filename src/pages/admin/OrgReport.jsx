...
import { Badge, Card, Col, Grid, Row, Select, StatCard } from '../../components/ui';
...
export default function OrgReport() {
  ...
  const teamData = managers.map((m) => {
    ...
    return { id: m.id, name: m.group, managerName: m.name, members: team.length, rating: Number(avg.toFixed(1)), eligible, health: teamHealth(team.flatMap((x) => goalsFor(x.id))) };
  });

  const eligibilityDist = [PROMOTION.ELIGIBLE, PROMOTION.APPROACHING, PROMOTION.NOT_ELIGIBLE].map((tier) => ({
    name: tier.replace('_', ' '),
    value: state.users.filter((u) => u.role === ROLES.EMPLOYEE && eligibilityFor(u.id).tier === tier).length,
  ...
  return (
    <>
      ...
      <Grid columns={2} minWidth={360} style={{ marginBottom: 20 }}>
        ...
        <Card hoverable={false}>
          <h3 style={{ color: C.text, fontSize: 16, marginBottom: 10 }}>Promotion eligibility</h3>
          <div style={{ height: 260 }}>
            <ResponsiveContainer>
              <PieChart>
      ...
      <Card hoverable={false}>
        ...
        <Col gap={10}>
          {filteredTeams.map((t) => (
            <Row key={t.id} style={{ justifyContent: 'space-between', padding: 12, background: C.surface, borderRadius: 10 }}>
              <Col gap={2}>
                <div style={{ color: C.text, fontWeight: 600 }}>{t.name}</div>
                <div style={{ color: C.textMuted, fontSize: 12 }}>Manager: {t.managerName} · {t.members} members</div>
              </Col>
              <Row gap={8}>
